...
export default function ChatScreen() {
    ...
    switch(currentStage) {
      case 'initial':
        ...
        if (skipCityQuestion && userContext.city) {
          setUserContext(prev => ({ ...prev, conversationStage: 'askInterests' }));
      ...
      case 'askInterests':
        ...
        // Find schools based on collected information
  ...
  const findSchoolsBasedOnContext = () => {
    ...
    schoolsData.forEach((school, index) => {
      ...
      // Check if any interest keyword matches the school
      const interestMatch = interestKeywords.length === 0 || 
                           interestKeywords.some(keyword => schoolName.includes(keyword.toLowerCase()));
    ...
    // If no schools match both criteria, just show schools from that city
  ...
  const findRelevantSchools = (keywords) => {
    ...
    schoolsData.forEach((school, index) => {
      ...
      const cityMatch = !userContext.city || 
                        schoolCity === userContext.city.toLowerCase();
      
      // Check if any keyword matches the school
      const keywordMatch = lowerKeywords.length === 0 || 
  ...
  const qaMap = [
    ...
    },
    {
      ...
      response: 'Naravno, tu sam da ti pomognem! Koje te područje najviše zanima?',
      schoolKeywords: []
    },
    {
      keywords: ['matematika', 'matematiku'],
      response: 'Odlično! Ako te zanima matematika, preporučam ti prirodoslovno matematičku gimnaziju.',
      schoolKeywords: ['matematička', 'gimnazija', 'prirodoslovna']
    },
    {
      keywords: ['nisam siguran', 'ne znam'],
      response: 'To je u redu. Tu sam da ti pomognem, reci mi što te zanima ili u kojem gradu tražiš školu?',
      schoolKeywords: []
    },
    {
    ...
    },
    {
      ...
      response: 'To je super! Jako bi se dobro uklopio u srednju glazbenu školu.',
    ...
    },
    {
      ...
      response: 'Izvrsno! Savjetujem ti da pogledaš elektrotehničke škole s računalnim usmjerenjem.',
      schoolKeywords: ['tehnička', 'elektrotehnička', 'računalstvo']
    },
    {
      keywords: ['umjetnost', 'dizajn', 'crtanje'],
      response: 'Super! Preporučam ti škole likovnih umjetnosti i dizajna, gdje možeš razvijati kreativne vještine.',
      schoolKeywords: ['umjetnička', 'likovna', 'dizajn']
    },
    {
      keywords: ['medicina', 'doktor', 'medicinska'],
      response: 'Odlično! Potraži gimnazije s prirodoslovno-matematičkim programom ili medicinske škole.',
      schoolKeywords: ['medicinska', 'zdravstvena', 'gimnazija']
    },
    {
      keywords: ['praktične', 'radionice', 'praksa'],
      response: 'Razumijem – razmisli o srednjoj strukovnoj školi, gdje je puno praktičnog rada.',
      schoolKeywords: ['strukovna', 'tehnička', 'obrtnička']
    },
    {
      keywords: ['prijava', 'prijavljuje', 'upisi'],
      response: 'Prijava se obavlja elektroničkim putem preko stranice e-Upisi, a proces počinje u svibnju – sve detalje pronađi na https://e-upisi.skole.hr.',
      schoolKeywords: []
    },
    {
    ...
    switch(userContext.conversationStage) {
      case 'initial':
        ...
        if (detectedCity) {
          ...
          setUserContext(prev => ({ ...prev, city: detectedCity, conversationStage: 'askInterests' }));
      ...
      case 'askInterests':
        ...
        if (detectedInterests.length > 0) {
          setUserContext(prev => ({ 
            ...prev, 
            interests: [...new Set([...prev.interests, ...detectedInterests])] 
          }));
      ...
      case 'recommend':
        ...
        if (newCity && newCity !== userContext.city) {
          setUserContext(prev => ({ ...prev, city: newCity, conversationStage: 'askInterests' }));
        ...
        if (text.toLowerCase() === "zagreb") {
          setUserContext(prev => ({ ...prev, city: "Zagreb", conversationStage: 'askInterests' }));
          moveToNextStage('askCity');
          break;
        }
        
        if (text.toLowerCase() === "varaždin") {
          setUserContext(prev => ({ ...prev, city: "Varaždin", conversationStage: 'askInterests' }));
          moveToNextStage('askCity');
          break;
        }
